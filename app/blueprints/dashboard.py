...
from app.utils.db_consultas import consulta_dados_respostas, consulta_dados_gestor
...
def dashboard_view():    
    ...
    nota_geral = processa_respostas(respostas)
    nota_nps = processa_respostas(respostas, None, 53) #fk_pergunta
    nota_eficacia = processa_respostas(respostas, 2) #fk_categoria
    ...
    dados_main_cards = [{
            ...
            'nota_media_eficacia': nota_eficacia[0],
            'size_bar_eficacia':nota_eficacia[1],
            'qtd_respostas_eficacia': nota_eficacia[2],
            'data_min_eficacia':nota_eficacia[3],
            'data_max_eficacia': nota_eficacia[4],
        }]
    
    cards = gera_cards(respostas)
    
    return render_template('dashboard.html', perfil = perfil, dados=dados_main_cards, cards=cards)
...
def detalhes_categoria_view(card_id):
    ...
    app.logger.debug(cards_perguntas)
    if cards_perguntas:
